...
findOldestCatHouse houses =
  ...
  where housesSortedByCatAge = L.sortBy catAgeComparer houses
...
main = putStrLn oldest
  where oldest = case findOldestCatHouse street of
                   Nothing -> "There is no oldest cat"
                   Just house -> "the oldest cat is " 
